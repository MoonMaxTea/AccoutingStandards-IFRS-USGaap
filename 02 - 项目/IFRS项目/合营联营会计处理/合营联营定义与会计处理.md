...
> **TL;DR**：合营以**共同控制**为前提（IFRS 11），分合营经营（按比例确认）与合营企业（权益法）；联营以**重大影响**为前提（IAS 28），统一权益法。典型情形：与第三方 **50:50 设立公司**且已确认共同控制 → 存在**单独载体** → 通常为**合营企业** → **权益法**（非合并）。
> **关键依据**：IFRS 11 §7–8（共同控制）、§15–16（分类）、§24（合营企业权益法）；IAS 28 §16；Example 4（50:50 设公司）
> **适用场景**：判断投资分类、选择会计处理方法、与子公司合并范围区分时；50:50 合营公司设立后的分类与入账
...
| 待决事项 | 合营经营 vs 合营企业？是否合并？ |

### 决策问题

1. 该安排属于合营经营还是合营企业？
2. 投资者应采用合并、权益法还是其他处理？

### 判断链

```mermaid
flowchart TD
    A[50:50 设立法人公司] --> B{是否共同控制?<br>已确认：一致同意}
    B -->|是| C{是否通过单独载体?<br>法人公司 = 是}
    C -->|是| D{权利义务类型<br>IFRS 11 §14-18, B22-B33}
    D -->|对净资产有权利<br>默认情形| E[合营企业<br>IFRS 11 §16]
    D -->|合同修改为对资产/负债<br>直接权利义务| F[合营经营<br>IFRS 11 §15]
    E --> G[权益法 IAS 28 §16<br>IFRS 11 §24]
    F --> H[按比例确认<br>IFRS 11 §20-22]
    B -->|否| I[再评估重大影响等]
```
...
| **联营 + 权益法** | 否（分类上） | 已确认**共同控制**，优先适用 IFRS 11；处理同为权益法但定性不同 |
| **合营企业 + 权益法** | **是（默认结论）** | IFRS 11 §24 明确要求 |
...
| **50:50 设公司 + 共同控制已确认** | 单独载体 → 默认**合营企业** → **权益法**；非合并（见 §五） |
...
- **单独载体**指是否经独立法人/财务结构运作，与「资产出资」无关（见 §1.2）
...
- 2026-06-18：新增 §五 实务案例（50:50 设公司、共同控制已确认 → 合营企业 → 权益法）；补充单独载体术语澄清
