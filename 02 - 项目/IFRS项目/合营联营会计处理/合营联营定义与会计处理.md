---
tags: [ifrs, ifrs11, ias28, 合营, 联营, 权益法, 共同控制, 重大影响, 分类, 单独载体, 实务案例]
date: 2026-06-18
status: active
type: A-概念梳理
...
> **TL;DR**：合营以**共同控制**为前提（IFRS 11），分合营经营（按比例确认）与合营企业（权益法）；联营以**重大影响**为前提（IAS 28），统一权益法。本案（50:50 设公司、共同控制已确认）→ 单独载体 → 默认**合营企业** → **权益法**（非合并）。
> **关键依据**：IFRS 11 §7–8（共同控制）、§15–16（分类）、§24（合营企业权益法）；IAS 28 §16；Example 4（50:50 设公司）
...
| 事实 | 说明 |
|------|------|
| 架构 | 本公司与第三方各出资 **50%**，共同设立一家**法人公司** |
| 表决权 | 双方各持 50% 股权/表决权 |
| 已确认事项 | 相关活动决策须双方**一致同意** → **共同控制**已成立 |
| 待决事项 | 投资如何分类？采用何种会计处理？ |

在 IFRS 体系下，投资者持有其他主体权益时，须先判断属于子公司、合营安排还是联营企业，再确定会计处理方法。本案是这一判断链条的典型入口。

### 决策问题

1. 该安排属于**合营经营**还是**合营企业**？是否应**合并**？
2. 投资者应采用**权益法**、按比例确认还是其他处理？
3. （延伸）**合营**与**联营**分别如何定义？二者在权力层级与处理上有何差异？
...
| 本案待解问题 | 对应章节 | 核心概念 |
|-------------|---------|---------|
| 属于哪一层级？（子公司 / 合营 / 联营 / 一般投资） | [[#总体框架]]、[[#四、判断流程]] | 控制 → 共同控制 → 重大影响 |
| 是否合营安排？「共同控制」如何认定？ | [[#一、合营（Joint Arrangement）\|§一.1]] | 一致同意（IFRS 11 §7–8） |
| 合营经营还是合营企业？ | [[#一、合营（Joint Arrangement）\|§一.2–3]] | 单独载体、权利义务类型（IFRS 11 §14–18） |
| 与联营如何区分？ | [[#二、联营（Associate）]]、[[#三、合营 vs 联营：对比总结]] | 共同控制 vs 重大影响 |
| 权益法如何适用？ | [[#一、合营（Joint Arrangement）\|§一.3]]、[[#二、联营（Associate）\|§二.2]] | IAS 28 权益法 |
| 单独报表如何处理？ | [[#五、单独财务报表的处理差异]] | IAS 27 §10 |
| 本案最终结论 | [[#结论]] | 合营企业 + 权益法，非合并 |
...
**回应问题 3**：先看 IFRS 如何将「对其他主体的权益」按**控制程度**分层——本案 50% 持股本身不能定性强弱，须先定位所在层级。

| 层级 | 关键概念 | 主要准则 | 典型会计处理 | 与本案关系 |
|------|---------|---------|-------------|-----------|
| 子公司 | **控制**（Control） | IFRS 10 | 合并财务报表 | ❌ 50% + 须一致同意 → 无单方控制 |
| 合营安排 | **共同控制**（Joint Control） | IFRS 11 | 合营经营：按比例确认；合营企业：权益法 | ✅ 已确认共同控制 |
| 联营企业 | **重大影响**（Significant Influence） | IAS 28 | 权益法 | ❌ 本案优先适用 IFRS 11 |
...
**回应问题 1–2（上）**：本案已确认共同控制，属于合营安排。以下分三步：认定共同控制 → 区分经营/企业 → 确定处理方法。
...
| 原则 | 原文依据 | 本案 |
|------|---------|------|
| 合营安排 | IFRS 11 §4–6：两方共同控制 | ✅ 合营安排 |
| 一致同意 | IFRS 11 §7–8：unanimous consent | ✅ 已确认 |
| 非控制 | IFRS 10：单方无法控制 | ✅ 不合并 |
...
**回应问题 1（核心）**：在合营安排已成立的前提下，IFRS 11 不按法律形式分类，而按参与方对安排的**权利与义务**分类。
...
| 概念 | 含义 | 常见误区 |
|------|------|---------|
| **单独载体**（Separate Vehicle） | 合营安排是否通过**独立可识别的财务结构**运作（如法人公司、信托等），使载体自身持有资产、承担负债 | 误将「以资产出资」等同于「存在单独载体」 |
| **资产出资** | 参与方投入资产的方式 | 可在有或无单独载体的安排中均出现 |

单独载体问的是**架构形式**（是否经独立法人/财务结构隔离），与出资方式是**不同维度**的判断。

**本案分类分析**（IFRS 11 Example 4）：

> **IFRS 11 Example 4（知识库原文）：**
>
...
| 判断步骤 | 准则依据 | 本案结论 |
|---------|---------|---------|
| 单独载体 | IFRS 11 B19–B22：法人公司使资产/负债归属公司本身 | ✅ 存在单独载体 |
| 分类（默认） | Example 4 前半段：法律形式赋予对**净资产**的权利 | ✅ **合营企业** |
| 例外路径 | Example 4 后半段：合同修改使各方对资产/负债有直接权利义务 | ⚠️ 须核查合同是否如此约定 |

| 易混淆概念 | 本案 | 说明 |
|-----------|------|------|
| **子公司 + 合并** | 否 | 50% 且须一致同意 → 无 IFRS 10 意义上的**控制** |
| **合营经营 + 按比例确认** | 通常否 | 法人公司 + 默认合同 → 对**净资产**而非资产/负债直接享有权利 |

### 3. 合营的会计处理

**回应问题 2**：分类确定后，处理方法随之确定。
...
> **本案**：默认路径下**不适用**；仅当合同条款触发 Example 4 后半段例外时改按本段处理。
...
**回应问题 3（延伸）**：若不存在共同控制，但存在重大影响，则归入联营。本案已确认共同控制，**不适用**联营分类，但联营的权益法处理与合营企业相同，故一并梳理。
...
> **与本案**：50% + 一致同意 = **共同控制**，优先适用 IFRS 11，定性为合营而非联营；但处理上同为权益法（见 §二.2）。
...
**回应问题 3**：合营与联营的核心差异一览——本案落在「合营 → 合营企业」象限。

| 维度 | 合营（IFRS 11） | 联营（IAS 28） | 本案 |
|------|----------------|---------------|------|
| **权力层级** | 共同控制 | 重大影响 | 共同控制 |
| **决策要求** | 相关活动须一致同意 | 参与财务和经营决策，但非控制/共同控制 | 一致同意 |
| **分类** | 合营经营 / 合营企业 | 单一类别（联营企业） | 合营企业 |
| **合营经营处理** | 按比例确认资产、负债、收入、费用 | — | 不适用 |
| **合营企业处理** | 权益法（IAS 28） | 权益法（IAS 28） | ✅ 权益法 |
| **表决权推定** | 不适用简单比例推定，须评估一致同意机制 | ≥20% 推定有重大影响 | 须证明一致同意 |
...
**回应问题 1–3（汇总）**：以下流程可用于复盘本案及类似投资分类。
...
**本案路径**：A → B（✅ 一致同意）→ C（✅ 单独载体 + 默认合同）→ E（合营企业 → 权益法）。

---

## 五、单独财务报表的处理差异

| 投资类型 | 合并/主财务报表 | 单独财务报表 | 本案 |
|---------|---------------|-------------|------|
| 合营经营 | 按比例确认（IFRS 11 §20–22） | 同左（IFRS 11 §26(a)） | — |
| 合营企业 | 权益法（IFRS 11 §24 → IAS 28） | 按 IAS 27 §10（成本法或 IFRS 9 或权益法） | ✅ 主表权益法 |
...
| 情形 | 建议处理 |
|------|---------|
| **本案：50:50 设公司 + 共同控制已确认** | 单独载体 → 默认**合营企业** → **权益法**；**不合并** |
...
| 项目 | 建议 |
|------|------|
| **投资分类** | 合营安排 → **合营企业**（Joint Venture） |
| **合并报表** | **不合并**该被投资公司 |
| **会计处理** | 对合营企业投资按 **IAS 28 权益法**核算 |
| **底稿留档** | 记录共同控制分析（一致同意机制）、单独载体判断、合同条款核查（排除 Example 4 后半段例外） |
| **后续复核触发点** | 合同修订导致资产/负债直接归属变化；一方取得单方面主导权 |

## 实务提示

- **单独载体**指是否经独立法人/财务结构运作，与「资产出资」无关（见 §一.2）
- 合营分类不能只看法律形式，须综合合同条款与事实（IFRS 11 B12–B33）；Example 4 表明合同可**逆转**法人形式下的默认分类
- 持股 ≥20% 仅是对联营的**推定**，合营须证明**一致同意**机制
- 50:50 设公司时，共同控制成立 ≠ 可以合并；合并仅适用于 IFRS 10 **控制**
...
| 准则 | 文件 | 核心段落 |
|------|------|---------|
| IFRS 11 | [[03 - 知识库/IFRS/IFRS准则/IFRS 11 - Joint Arrangements\|IFRS 11 - Joint Arrangements]] | §4–27（定义与处理）；Appendix B（应用指南）；**Example 4**（50:50 设公司） |
...
- 2026-06-18：初稿，基于知识库 IFRS 11、IAS 28 整理
- 2026-06-18：对齐项目编写说明（TL;DR、type A、分层结论、实务提示）
- 2026-06-18：新增 50:50 设公司实务案例；补充单独载体术语澄清
- 2026-06-18：重构为「问题驱动」结构——案例前置至「问题」节，各板块由本案逐层引出
