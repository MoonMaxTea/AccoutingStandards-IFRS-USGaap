...
规范**存货**的确认、计量和成本流转假设，是制造业和零售业的基础准则。

### 计量基础
**成本与可变现净值孰低（Lower of Cost and NRV）**

### 存货成本构成
| 成本要素 | 内容 |
|----------|------|
| **采购成本** | 购买价格 + 进口关税 + 运费 - 折扣/补贴 |
| **加工成本** | 直接人工 + 系统分配的固定/变动制造费用 |
| **其他成本** | 使存货达到当前地点和状态的必要支出 |

### 成本流转假设
| 方法 | 适用场景 | 要点 |
|------|----------|------|
| **个别辨认法** | 不可替代的特定项目 | 必须使用 |
| **先进先出法（FIFO）** | 多数存货 | 允许 |
| **加权平均法** | 多数存货 | 允许 |
| **后进先出法（LIFO）** | — | ❌ **IFRS 禁止** |

### 可变现净值（NRV）
- 正常经营中估计售价 − 完工尚需成本 − 销售费用
- **逐项计提**跌价准备
- 减记因素消失时可**转回**（恢复至原减记金额为限）

### 确认时点
- 出售时确认收入，同时将存货结转至营业成本

> 与 IAS 16（自建资产的资本化）、IFRS 15（收入确认时点）联动。US GAAP 差异：允许 LIFO、跌价不可转回。
...
Instruments (issued July 2014), IFRS 16 Leases (issued January 2016) and IFRS 18  Presentation
and Disclosure in Financial Statements (issued April 2024).
IAS 2
© IFRS Foundation A1155
...
The amount of inventories recognised as an expense during the period, which
...
Some entities classify expenses by nature in the operating category of the
statement of profit or loss in a way that results in amounts being presented
other than the cost of inventories recognised as an expense during the period.
In this case, the entity presents the costs recognised as an expense for raw
35
36
37
38
39
IAS 2
A1164 © IFRS Foundation

materials and consumables, employee benefits and other costs together with
...
amended paragraph 39. An entity shall apply those amendments when it
applies IFRS 18.
