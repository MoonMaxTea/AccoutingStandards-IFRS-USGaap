---
tags: [ifrs, ifrs-4, 保险, 被取代]
standard: IFRS 4
title: "保险合同（已被 IFRS 17 取代）"
effective: 2005-01-01（被 IFRS 17 取代）
source: IFRS Foundation
---

# IFRS 4 — Insurance Contracts（保险合同，旧）

## 📌 中文提炼

允许各种本地保险会计实践，要求有限。不再详述。
...
In December 2005 the Board issued revised guidance on
implementing IFRS 4.
In September 2016 IFRS 4 was amended by Applying IFRS 9 Financial Instruments with IFRS 4
Insurance Contracts.
These amendments address concerns arising from the different
...
Other Standards have made minor consequential amendments to IFRS 4.
They include
...
All the paragraphs have equal authority.
...
Furthermore, those requirements also apply if the
...
component.
In some cases, an insurer is required or permitted to unbundle those
components:
(a)
...
requirements, this IFRS imposes no further requirements.
The minimum
...
than IFRS 9 for annual periods beginning before 1 January 2021.
An
...
arising from, the contracts in (a) and (b).
...
For example, if an entity determines that it no longer
...
this IFRS.
For example, the requirements and exemptions in IFRS 1 do not
...
estimates and assumptions for the designated liabilities.
...
However, if an insurer already measures its
...
Two examples of accounting policies that reflect
those margins are:
(a)
...
However, in other approaches, the discount rate
...
An insurer is permitted, but not
...
This IFRS does not specify how the
...
or loss.
If part or all of the discretionary participation feature is
...
The issuer shall recognise the portion of
...
An insurer that has elected to apply the overlay
...
'shadow accounting'.
If an insurer applies the overlay approach, shadow
...
amounts in other comprehensive income, such as income taxes.
An insurer
...
Disclosures about the temporary exemption from IFRS 9
39B
An insurer that elects to apply the temporary exemption from IFRS 9 shall
...
individually immaterial associates or joint ventures.
The aggregate
amounts:
(i)
disclosed shall be the entity's share of those amounts; and
...
paragraphs 4(a) and (c), B7, B18(h) and B21.
...
first financial year in which it applies this IFRS.
...
(See Appendix B for guidance on this
definition.)
insurance liability
...
compensate another insurer (the
cedant) for losses on one or
...
financial effect is still uncertain.
An example is a reinsurance contract that
covers the direct insurer against adverse development of claims already reported
by policyholders.
In such contracts, the insured event is the discovery of the
...
not regulated as insurance contracts in some countries.
One example is a
...
criteria).
That approach is also acceptable under this IFRS, which
...
paragraphs 15-19 of this IFRS.
If this IFRS did not apply to these
...
payment that is contingent on the insured event can be significant.
For
...
also transfers insurance risk.
If the resulting transfer of insurance risk is
...
policyholder.
In other words, insurance risk is a pre-existing risk transferred
...
to mitigate an underlying risk exposure.
For example, if the holder uses a
...
the policyholder is a contractual precondition for payment.
This contractual
...
an uncertain future event that adversely affects the counterparty.
Similarly,
...
insurance risk.
However, if the issuer of that contract mitigates that risk by
...
default contract or an insurance contract.
...
(h)
product warranties.
Product warranties issued by another party for
...
Those additional amounts include claims
...
For example, consider a contract that
...
These additional benefits could
...
reference to the insurance component.
