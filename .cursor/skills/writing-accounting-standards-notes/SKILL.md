---
name: writing-accounting-standards-notes
description: Use when the user asks IFRS or US GAAP accounting questions and wants project notes created in this Obsidian vault, including definition, classification, measurement, disclosure, dual-standard comparison, or practical decision analysis based on the workspace knowledge base.
---

# 准则项目笔记编写

## Overview

IFRS / US GAAP 问题 → **结构化项目笔记**，存于 `02 - 项目/`，准则**仅限** `03 - 知识库/`。

**完整规范**：[[02 - 项目/项目编写说明]]（现行唯一规范，无版本号）| **索引**：[[02 - 项目/项目索引]]

用户说「按项目编写说明写」→ 直接按该文件最新内容执行，无需加版本号。

## Quick Reference

| 类型 | 目录 | 适用 |
|------|------|------|
| A | `IFRS项目/` 或 `USGAAP项目/` | 概念梳理 |
| B | `IFRS项目/` 或 `USGAAP项目/` | 实务决策 |
| C | `双准则对比/` | IFRS vs US GAAP |

## Frontmatter（必选）

```yaml
---
tags: [ias12, 递延所得税]
date: YYYY-MM-DD
status: active              # active | archived | needs-update
type: B                       # A | B | C
standards: [IAS 12]
related:
  - "[[适用税率选择及计算]]"
---
```

## 正文必选结构

1. **TL;DR**（三行 blockquote：结论 / 依据 / 场景）
2. **问题**（背景 + 决策问题 + 边界可选）
3. **准则原文** + **提炼表**（每个主要引用块后）
4. **分析**
5. **结论**：`### 准则结论` + `### 操作结论`（B 型必选）
6. **相关项目** / **准则索引** / **日志**（含变更原因）
7. 更新 **[[02 - 项目/项目索引]]**

## 引用格式

```markdown
> **IAS 12 Paragraph 24（知识库原文）：**
> [英文]

| 原则 | 原文依据 | 提炼 |
|------|---------|------|

[[03 - 知识库/IFRS/IAS准则/IAS 12 - Income Taxes|IAS 12]]
```

- 中文提炼标注 `（知识库中文提炼）`，不可标为原文
- C 型加术语对照表

## 工作流程

1. 检索知识库 → 确认覆盖
2. 选 A/B/C 与目录
3. 复杂问题先列写作计划
4. 按结构撰写
5. 更新项目索引
6. 自检 [[02 - 项目/项目编写说明#九、章节速查清单]]

## Common Mistakes

| 错误 | 正确 |
|------|------|
| 无 TL;DR | 标题下必有三行摘要 |
| 结论不分层 | 准则结论 vs 操作结论 |
| 无提炼表 | 引用块后默认加表 |
| 对比文放 IFRS项目 | C 型放 `双准则对比/` |
| 未更新索引 | 新建后更新项目索引 |

## Red Flags — STOP

- 知识库无准则却引用 → 标注缺失
- 外部准则来源 → 删除
- 无 Wiki 链接 → 补齐
